%function [ux2,uy2,ux,uy,p] = hmfem(K,mob,Nx,Ny,Lx,Ly,perm,layer,hx,hy)
...
%
%
% Nx = 60;  % Numero de pontos em x
% Ny = 220 ; % Numero de pontos em y
% M  = Nx*Ny; % Numero de elementos
% Lx  = 1200; % Tamanho do domínio em x
% Ly  = 2200; % Tamanho do domínio em y
% hx  = Lx/Nx; % Espacamento de malha em x
% hy  = Ly/Ny; % Espacamento de malha em y

 Nx =256;  % Numero de pontos em x
 Ny =64; % Numero de pontos em y
M  = Nx*Ny; % Numero de elementos
Lx  =256; % Tamanho do domínio em x
Ly  = 64; % Tamanho do domínio em y
% hx  = Lx/Nx; % Espacamento de malha em x
% hy  =Ly/Ny; % Espacamento de malha em y
...
for i = 1:Nx-1
  for j = 1:Ny-1
    Dx(i,j) = (1/hx).*(ux(i+1,j) - ux(i,j));
    Dy(i,j) = (1/hy).*(uy(i,j+1) - uy(i,j));
...
quiver(xx,yy,ux,uy,'LineWidth',1,'AutoScaleFactor',20);
axis equal tight;
xlabel ("y");
ylabel ("x");
