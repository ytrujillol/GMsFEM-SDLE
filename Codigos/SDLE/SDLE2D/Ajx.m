...
B12(Nx,j) =B12(Nx-1,j);%u0(Nx,j) -u0(Nx-1,j);
   ...
   switch ordem
    ...
    case 1
      ...
      vx(1,j) =vx(2,j) ; % MM(A12(1,j),B12(1,j));
      vx(Nx,j)=vx(Nx-1,j) ;% MM(A12(Nx,j),B12(Nx,j));
...
%        % minmod3
 ...
 vx(1,j) =vx(2,j); % MM3(a*A12(1,j),0.5*C2(1,j),a*B12(1,j));
 vx(Nx,j)=vx(Nx-1,j); % MM3(a*A12(Nx,j),0.5*C2(Nx,j),a*B12(Nx,j));
